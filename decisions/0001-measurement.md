...
`plural()` allocates one object per call, by design, in the `plural()` body:
...
`{...params, count}` object in the `plural()` body riding on top of the same returned
...
(it names the compiled entry closure returned by `compileString`), not as a gate. Rejected as a
